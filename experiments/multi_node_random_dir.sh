...
#SBATCH --job-name=multi-nodes-random-dir
#SBATCH --output=logs/multi-nodes-random-dir-%j.out
#SBATCH --error=logs/multi-nodes-random-dir-%j.err
...
# This calls src/parallel_random_dir.py ONCE, which iterates the directory internally
# without restarting Ray.
#
# Args:
#   1: qv_dir (default "graphs")
#   2: results_dir (default "results_random_dir")
#   3: num_candidates X (default 0 -> per-instance default is n+1)
...
#
# Example:
#   sbatch experiments/multi_node_random.sh graphs/erdos_renyi/rank_1/p01/n20 results/random/erdos_renyi/n20 0 32 1000 0 3
#   sbatch experiments/multi_node_random.sh graphs/erdos_renyi/rank_1/p01/n20 results/random/erdos_renyi/n20 5000 32 1000 0 3

set -euo pipefail

QV_DIR=${1:-graphs}
RESULTS_DIR=${2:-results_random_dir}
NUM_CANDIDATES=${3:-0}          # 0 means default per-instance X = n+1
PRECISION=${4:-64}
...
echo "Using num_candidates = $NUM_CANDIDATES (0 => per-instance default n+1)"
...
# parallel_random_dir.py will:
# - ray.init(address="auto") once
# - iterate all Q*.npy / V*.npy pairs
# - run random baseline with X candidates per instance (default n+1 if X=0)
...
srun --nodes="$SLURM_JOB_NUM_NODES" --ntasks="$SLURM_JOB_NUM_NODES" \
  python -m read_only.symmetric_run \
    ...
    -- \
    python -u src/parallel_random_dir.py \
      --qv_dir "$QV_DIR" \
