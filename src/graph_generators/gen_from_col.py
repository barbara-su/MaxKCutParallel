...
def main():
    ...
    parser.add_argument(
        "--col", type=str, default="anna",
        help="Identifier of col graph."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--in_dir", type=str, default="../graphs/color",
        help="Directory containing COLOR graphs."
    )
    parser.add_argument(
    ...
    with open(f"{in_dir}/{col}.col") as f:
        line = next(f).split()
        while line[0] != "p":
            line = next(f).split()

        num_nodes = int(line[2])
        n = num_nodes
        G.add_nodes_from(range(num_nodes))
        for line in f:
            i, j = map(int, line.split()[1:3])
            if random_weights:
                G.add_edge(i - 1, j - 1, weight=1 * ((random_high - random_low) * np.random.rand() + random_low))
            else:
                G.add_edge(i - 1, j - 1, weight=1)
                
    # generate graph
    print(f"Using {col} from COLOR, n = {n}, seed = {seed}, rank = {r}")
    ...
    # save
    q_path = os.path.join(out_dir, f"Q_COLOR_{col}.npy")
    v_path = os.path.join(out_dir, f"V_COLOR_{col}.npy")
