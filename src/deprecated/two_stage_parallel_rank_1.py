...
# -----------------------------
@ray.remote
def generate_l_batches(num_candidates, candidates_per_task, start_batch_id, start_l):
    """
    Generate batches of l-values starting from start_l.
    Each batch has size candidates_per_task (except possibly the last).
    Returns a list of (batch_id, batch_l_values).
    """
    batches = []
    iterator = iter(range(start_l, num_candidates))
...
def process_rank_1_parallel(V, Q, K=3, candidates_per_task=10, X=5.0):
    """
    Rank-1 parallel sweep over l = 0..n with a two-stage design.

    X% of CPUs are reserved for candidate generation.
    (100-X)% of CPUs are reserved for scoring.
    Both stages use an in-flight cap.
    """
    n = V.shape[0]
    ...
    if not (0.0 < X < 100.0):
        raise ValueError("--generator_percent (X) must be in (0, 100)")

    # initial assignment (same as original)
    ...
    num_candidates = n + 1
    total_batches = (num_candidates + candidates_per_task - 1) // candidates_per_task

    resources = ray.available_resources()
    num_cpus = max(1, int(resources.get("CPU", 1)))

    gen_cpus = max(1, int(num_cpus * (X / 100.0)))
    score_cpus = max(1, num_cpus - gen_cpus)

    log.info(f"Total candidates (prefix lengths): {num_candidates}")
    log.info(f"Detected CPUs: {num_cpus}")
    log.info(f"Generator CPUs: {gen_cpus} (X={X}%), Scorer CPUs: {score_cpus}")
    log.info(f"candidates_per_task: {candidates_per_task}")
    log.info(f"Total Ray scoring batches: {total_batches}")
    ...
    # -----------------------------
    # Stage 1: generate batches WITH in-flight cap
    # -----------------------------
    # We generate "chunks" of batches. Each generator task generates several batches.
    batches_per_gen = (total_batches + gen_cpus - 1) // gen_cpus

    def gen_chunks():
        g = 0
        while True:
            start_batch = g * batches_per_gen
            start_l = start_batch * candidates_per_task
            if start_l >= num_candidates:
                break
            yield start_batch, start_l
            g += 1

    all_batches = []

    gen_max_in_flight = gen_cpus
    gen_in_flight = []
    gen_iter = iter(gen_chunks())

    for _ in range(gen_max_in_flight):
        try:
            start_batch, start_l = next(gen_iter)
        except StopIteration:
            break
        gen_in_flight.append(
            generate_l_batches.options(num_cpus=1).remote(
                num_candidates,
                candidates_per_task,
                start_batch,
                start_l,
    ...
    while gen_in_flight:
        done, gen_in_flight = ray.wait(gen_in_flight, num_returns=1)
        batches = ray.get(done[0])
    ...
    log.info(f"Generated {len(all_batches)} scoring batches")

    # -----------------------------
    # Stage 2: score batches WITH in-flight cap
    # -----------------------------
    best_score = float("-inf")
    best_l = 0

    score_max_in_flight = score_cpus
    in_flight = []
    iterator = iter(all_batches)

    for _ in range(min(score_max_in_flight, len(all_batches))):
        b_id, batch = next(iterator)
        in_flight.append(
            process_rank_1_batch.options(num_cpus=1).remote(
                batch, k0_ref, sorted_idx_ref, Q_ref, roots_ref, K, b_id
            )
        )
    ...
    log.info(f"Submitted={submitted}, completed={completed}, gen_cpus={gen_cpus}, score_cpus={score_cpus}")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="How many candidates (l values) each scorer task evaluates serially (default: 10)."
    )
    parser.add_argument(
        "--generator_percent",
        type=float,
        default=5.0,
        help="X: percent of CPUs reserved for candidate generation (default: 5.0)."
    )
...
def main():
    ...
    log.info("Executing parallel rank 1 algorithm (split gen/score + in-flight caps)")
    start = time.time()
    ...
    log.info(f"generator_percent={args.generator_percent}")
    ...
    output = {
        ...
        "candidates_per_task": args.candidates_per_task,
        "generator_percent": args.generator_percent,
        "best_l": best_l,
    ...
    filename = f"{timestamp}_result_n{args.n}_r1_p{args.precision}_cpt{args.candidates_per_task}_x{int(args.generator_percent)}.json"
