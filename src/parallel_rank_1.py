...
def process_rank_1_parallel(V, Q, K=3, candidates_per_task=10):
    ...
    candidates_per_task controls how many candidates each Ray task evaluates
    serially (the size of l_values).
    """
    n = V.shape[0]
    ...
    # each worker handles one task
    num_candidates = n + 1
    total_tasks = (num_candidates + candidates_per_task - 1) // candidates_per_task  # ceil
