...
def log_worker_memory(stage: str, l: int) -> None:
    """
    Log current memory usage of this worker process.

    ru_maxrss is in kilobytes on Linux, so convert to MiB.
    """
    usage = resource.getrusage(resource.RUSAGE_SELF)
    rss_mb = usage.ru_maxrss / 1024.0
    log.info(f"[worker l={l}] {stage}: RSS ~ {rss_mb:.1f} MiB")


@ray.remote
def process_rank1_candidate(l, k0, sorted_idx, Q, roots, K):
    try:
        # optional throttling if you do not want spam:
        # only log for some l values
        if l == 0 or l % 5000 == 0:
            log_worker_memory("start", l)

        k = k0.copy()
        if l == 0 or l % 5000 == 0:
            log_worker_memory("after k copy", l)

        k[sorted_idx[:l]] = (k[sorted_idx[:l]] + 1) % K

        z = roots[k]
        if l == 0 or l % 5000 == 0:
            log_worker_memory("after z construction", l)

        score = np.real(z.conj() @ Q @ z)
        if l == 0 or l % 5000 == 0:
            log_worker_memory("after score computation", l)

        return score

    except MemoryError:
        # This only runs if Python raises MemoryError
        # If the OS kills the process hard, you see the last stage log only
        log_worker_memory("MemoryError caught", l)
        log.exception(f"[worker l={l}] MemoryError in process_rank1_candidate")
        raise
