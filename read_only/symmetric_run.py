...
CLUSTER_WAIT_TIMEOUT = 180 # env_integer("RAY_SYMMETRIC_RUN_CLUSTER_WAIT_TIMEOUT", 30)
...
def symmetric_run(address, min_nodes, ray_args_and_entrypoint):
    ...
    click.echo(f"[DEBUG] hostname={socket.gethostname()}")
    click.echo(f"[DEBUG] resolved_gcs_host={resolved_gcs_host}")
    click.echo(f"[DEBUG] my_ips={my_ips}")
    click.echo(f"[DEBUG] is_head={is_head}")
